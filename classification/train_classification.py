...
def adjust_alpha_cosine(optimizer, epoch, max_epochs, start_alpha=0.5, final_alpha=1.0, min_alpha=0.1, warmup_epochs=10):
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, args, model_ema=None, scaler=None):
    ...
    train_acc1 = metric_logger.meters["acc1"].global_avg
    train_acc5 = metric_logger.meters["acc5"].global_avg
...
def main(args):
    ...
    if args.amp:
        print('You are using amp mixed-precision')
    ...
    if opt_name.startswith("SGDM"):
        ...
        optimizer = torch.optim.RMSprop(
            parameters, lr=args.lr, momentum=args.momentum, weight_decay=args.weight_decay, alpha=0.9
        )
    elif opt_name == "AdamW":
        ...
        raise RuntimeError(f"Invalid optimizer {args.opt}. Check the supported optimizers")
    ...
    start_time = time.time()
    fields = ['epoch', 'train_loss', 'train_acc1', 'train_acc5', 'val_acc1', 'val_acc5', 'best_acc1', 'best_acc5', 'optimizer']

    # Create a logger instance
    logger = TrainingLogger(os.path.join(args.output_dir,f"training_log_{start_time}.csv"), fields)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.alpha_scheduler:
            adjust_alpha_cosine(opt, epoch, args.epochs, start_alpha=args.alpha, final_alpha= 1.0, min_alpha=args.alpha,                                         warmup_epochs=0)
                    
        if args.beta_scheduler:
            adjust_beta_cosine(optimizer, epoch, epochs, start_beta=beta, final_beta=1.0, min_beta=beta, warmup_epochs=0)
        ...
        lr_scheduler.step()
        val_acc1, val_acc5 = evaluate(model, criterion, data_loader_test, device=device)
        ...
        if args.output_dir:
            ...
            best_acc5 = max(best_acc5, val_acc5)
            
            if args.opt.startswith('INNA') or args.opt.startswith('DIN'):
                log_data = {
                'epoch': epoch,
                ...
                'best_acc1': best_acc1,
                'best_acc5': best_acc5,
                "optimizer":  f"{args.opt}, ($\\alpha$, $\\beta$)= ({args.alpha},{args.beta}), lr = {args.lr}, wd = {args.weight_decay}"
                }
                ...
                'best_acc1': best_acc1,
                'best_acc5': best_acc5,
                "optimizer": f"{args.opt} , lr = {args.lr}, wd = {args.weight_decay}"
                }
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--data-path", default="data/imagenet/ILSVRC/Data/CLS-LOC", type=str, help="dataset path")
