...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16),
                 num_init_features=64, bn_size=4, drop_rate=0, num_classes=10, memory_efficient=False, norm_layer=None):
        ...
        for i, num_layers in enumerate(block_config):
            ...
            if i != len(block_config) - 1:
                trans = _Transition(num_input_features=num_features,
                                    num_output_features=num_features // 2, norm_layer=None, relu_fn=nn.ReLU)
...
def test():
    net = densenet121(maxpool_fn=lambda: MaxPool2dAlpha(0),relu_fn = lambda:ReLUBeta(0))
