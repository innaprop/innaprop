...
from optimizer import (
    ...
    create_innaprop_optimizer,
    create_optimizer_scheduler, 
    add_optimizer_params, 
    create_adam_optimizer_from_args,
    create_innaprop_optimizer_from_args
)
        ...
        if train_step % args.eval_interval == 0:
            ...
            if args.rank == 0:
                ...
                with open(filename, mode="a", newline="") as csv_file:
                    ...
                    if args.optimizer_name == 'INNAprop':
                        writer.writerow({"epoch": epoch, "train_step": train_step, "valid_loss": valid_loss, "best_val_ppl": best_val_ppl, "optimizer": f"{args.optimizer_name}, ($\\alpha$, $\\beta$)= ({args.innaprop_alpha},{args.innaprop_beta})", "lr": optimizer.param_groups[0]['lr']})
                    else:
                        writer.writerow({"epoch": epoch, "train_step": train_step, "valid_loss": valid_loss, "best_val_ppl": best_val_ppl, "optimizer": f"{args.optimizer_name}", "lr": optimizer.param_groups[0]['lr']})
...
if __name__ == '__main__':
    ...
    if args.optimizer_name == 'INNAprop':
        outdir = f"results/{args.model_card}/{args.optimizer_name}_alpha_{args.innaprop_alpha}_beta_{args.innaprop_beta}"
    elif args.optimizer_name == 'AdamW':
        outdir = f"results/{args.model_card}/{args.optimizer_name}_beta1_{args.adam_beta1}_beta2_{args.adam_beta2}"
    else:
    ...
    fieldnames = ["epoch", "train_step", "valid_loss", "best_val_ppl", "optimizer", "lr"]
    ...
    elif args.optimizer_name == 'INNAprop':  
        optimizer = create_innaprop_optimizer_from_args(lm_net, args)
    else:
        print('Invalid optimizer name.')
